...
(function(w) {
    ...
    function getMode() {
        ...
        return cookie('gameMode') ? parseInt('gameMode') : MODE_NORMAL;
    }
    ...
    function updatePanel() {
        if (mode === MODE_NORMAL) {
            ...
            GameTimeLayer.innerHTML = `总分 ${_gameScore}`;
        }
    }
    //使重试按钮获得焦点
    ...
    function shareText(cps) {
        ...
        if (cps <= 2) return '“搞事被宿管抓走，扣分”<br/>';
        if (cps <= 5) return '“三栋已成废墟[lay in ruins]”<br/>';
        if (cps <= 8)  return '“塔中已升天”<br/>';
        if (cps <= 10) return '“从此再无大塘头村”<br/>';
        return '“东莞都没了草”<br/>';
    }
    ...
    w.save_cookie = function() {
        const settings = ['username', 'message', 'keyboard'];
    ...
    const clickBeforeStyle = ('./static/image/ClickBefore.png');
    const clickAfterStyle = ('./static/image/AfterClicking.png');
